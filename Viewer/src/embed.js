...
const COMMANDS = {
  async loadPacks({ base, packs }) {
    ...
    // Vue's assetsVersion watcher is asynchronous. Embedded callers need the
    // structure index to be complete before the loadPacks reply so an
    // immediate listStructures request cannot observe a stale empty index.
    await useStructures().refresh()
  },
